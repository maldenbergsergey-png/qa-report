...
const targets = {
  mac: ["mac-arm64"].map(dir => `${dir}/QA Report Connect.app/Contents/Resources/app.asar`),
  win: ["win-unpacked"].map(dir => `${dir}/resources/app.asar`),
};
assert(targets[process.argv[2]], "Specify mac or win");
for (const relative of targets[process.argv[2]]) {
  ...
  for (const name of ["qa-report-agent.js", "jira-profiles.js", "jira-headers.js", "desktop/main.js", "desktop/network.js", "bootstrap/main.js", "bootstrap/updates.js", "bootstrap/release-key.pem", "bootstrap/policy.json"]) {
