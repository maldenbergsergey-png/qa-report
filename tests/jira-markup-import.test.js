...
test("Jira image options do not split an imported table row", () => {
  ...
  assert.match(row.cells[section.columns[2].id], /data-jira-options="thumbnail"/);
  assert.equal(row.cells[section.columns[3].id], "Окно закрывается корректно");
});

test("an unavailable Jira image remains in its source cell without a visible wrapper", () => {
  const { section, row } = onlyRow(
    [
