...
test('autostart sends the new value before rendering restores the previous snapshot', async () => {
  const nodes = new Map();
  const node = id => {
    if (!nodes.has(id)) nodes.set(id, { value: '', checked: false, classList: { toggle() {} }, listeners: {}, addEventListener(name, fn) { this.listeners[name] = fn; } });
    return nodes.get(id);
  };
  ...
  const context = { document: { getElementById: node, documentElement: { dataset: {} } }, URL,
    window: { agent: { onStatus() {}, state: async () => ({ ok: true, value: state }), autoStart: async enabled => {
