...
for (const mode of ['section', 'continuous', 'hierarchical']) {
  test(`${mode}: Jira markup, ADF and Excel keep document numbers around omitted rows and sections`, () => {
    ...
    const filteredAdf = context.generateAdfDocument(filteredOptions);
    assert.equal(filteredAdf.content.filter(node => node.type === 'table').length, 1);
    ...
    for (const sectionIds of [null, ['b', 'd']]) {
      ...
      const adf = context.generateAdfDocument({ sectionIds });
      const headings = adf.content.filter(node => node.type === 'heading').map(node => node.content[0].text);
      assert.ok(headings.includes('2. Second'));
      assert.ok(headings.includes('4. Fourth'));
      const labels = adf.content.filter(node => node.type === 'table')
        .flatMap(table => table.content.slice(1).map(row => row.content[0].content[0].content[0].text));
      assert.deepEqual(Array.from(labels), wanted);
    }
